	...
	public function add_prova()
	{
		...
		else
		{
			...
			$datahora         	= date("Y-m-d H:i:s");
				...
				//GRAVANDO NO LOG A PROVA ENVIADA

				gravarLogs($this->session->userdata('id_professor'),$this->session->userdata('nome'), $this->session->userdata('nivel_professor'), $this->session->userdata('nivel_coordenador') , $this->session->userdata('nivel_operador'),  $this->session->userdata('nivel_tercerizado'), $this->session->userdata('nivel_administrador')  , 'Prova enviada com sucesso');
				...
				$message .= "<h5>Copyright - Sistema de Envio de Provas</em></h5>";
